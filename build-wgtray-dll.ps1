...
# ---- 6b) embed a launcher icon INTO the DLL (no .ico file at runtime) ----
# Draw a 256px rounded blue tile with a knocked-out U+5DE5 ("gong" - the tray
# identity glyph), wrap as PNG-in-ICO, inject RT_ICON + RT_GROUP_ICON via
# UpdateResource. The shortcut points at "WgTray.dll,0" and Explorer shows
...
$gp.AddString([string][char]0x5DE5, $f.FontFamily, [int][System.Drawing.FontStyle]::Regular, [single]($S * 52 / 64), (New-Object System.Drawing.RectangleF(0, 0, $S, $S)), $sf)
...
$ok1 = [Wg.W]::UpdateResource($hRes, [IntPtr]3, [IntPtr]1, 0, $png, [uint32]$png.Length)
$ok2 = [Wg.W]::UpdateResource($hRes, [IntPtr]14, [IntPtr]1, 0, $groupBytes, [uint32]$groupBytes.Length)
$ok3 = [Wg.W]::EndUpdateResource($hRes, $false)
if (-not ($ok1 -and $ok2 -and $ok3)) { throw 'icon resource injection failed' }
Write-Output ("icon embedded into WgTray.dll (256px PNG-in-ICO, {0} bytes)" -f $png.Length)
# verify: blue tile + knocked-out glyph
...
$c1 = $chkBmp.GetPixel([int]($chkBmp.Width * 0.12), [int]($chkBmp.Height * 0.5))
$c2 = $chkBmp.GetPixel([int]($chkBmp.Width * 0.5), [int]($chkBmp.Height * 0.5))
