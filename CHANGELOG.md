...
活动 / 其他 五组共 30 个姿势, 每行给正面 / 正面斜45° / 侧面(左右) / 背面 / 背面斜45° 六个视角。

**做法**(全程对着图改, **不动渲染管线**):
1. **取样定色**(不靠眼睛猜): 从姿态图里直接读像素 -> 背毛 `#eb953c` / 里白 `#fdf2e0` / 鼻眼 `#0f1111` /
   舌 `#f1836c` / 耳内 `#fbd1aa`; 描边取"最暗的暖色像素" ≈ `#6b3d1a` —— 是**暖棕细线**,
   不是原来那条近黑粗边(`#2e1b13`, 用户早说过"像记号笔")。
2. **零件重画**(`art/dog.svg`, 仍 6 件): `head` 改成"大圆颅 + 一小段短吻"的**单一闭合轮廓**
   (原来是颅骨 + 独立吻两件, 吻太尖, 读起来像鸟嘴); **新增 `mask` 白脸罩**(颊 + **眼周** + 吻,
   顶上留一道窄橙 V) —— 柴犬那张脸全靠它, 而且**眼睛必须坐在白里**(第一版把 V 开太深, 白脸被劈成两半);
   `ear` 改成底宽:高 ≈ 0.8 的厚三角; `mouth`/`bridge`/`body` 跟着挪位。
3. **五官**(`art.rs`): 眼睛从"大圆眼 + 双高光"改成姿态图的**小黑豆 + 单点高光**(更小、更低、更分开);
   加上**眉上两点**(柴犬的"四眼"); 鼻从大圆鼻头改成**小圆角三角**; 描边粗细按新线重调。
4. 结构上只有一处改名: `Shapes.muzzle` → **`Shapes.mask`**(兜底 `build_mask`), 其余零件字段不变;
   6 件仍然"SVG 优先、逐件退回内置", 内置路径**同步改过** —— 所以初始化日志那行还是 `包围盒差 0(逐件一致)`。

**验证**: `verify-overlay.py` **21/21**(日志 `art: svg 用了 6/6 个零件 [body bridge ear head mask mouth],
与内置路径的包围盒差: 0(逐件一致)`) / `wg-court.py` 7/7 / `wg-rustpet-panel.py` 16/16 /
`pet-dll-plugin-test.py` 22/22; DLL 1,786,221 → **1,787,749 B**; `plugins\wgpet.dll` 与
`package\plugins\wgpet.dll` 已同步(重启 WgIme 生效)。

**新探针** `%TEMP%\wg-svg-parts.py`: 用 **Inkscape**(本机有)把 `dog.svg` 的零件摊成一张"纸样"PNG
(每件一格 + 名字 + 填充/描边)。改形状时**先看纸样再编译** —— 从整只狗的画面里猜"这条贝塞尔到底画成什么了"
太费劲, 这一步省了好几轮。

**踩到的坑**: 那个探针里 `re.search(r'd="([^"]+)"', tag)` 会**命中 `id="head"` 里的那两个字符 `d="`** ——
抠出来的"路径数据"其实是字符串 `head`(报错信息 `这条 path 抠不出坐标: 'head'` 一眼看穿)。
Rust 侧 `svgpath.rs::attr()` 有"前一个字符不能是字母/数字/-/_"的检查, 所以**没中招**;
探针改成 `(?:\s)d="` 即可。留档: 凡按属性名扫 XML 属性, 都要防"属性名是另一个属性名的后缀"。
...
对 65,232 的注入上限只剩 **4,976 B** —— 再写一条大规则就会被**截断**(正是第八十轮修掉的故障)。用户: "先压 agents.md"。

**做法**(沿用第八十/八十一轮那套, 零信息损失): 挑最肥的 8 条 —— §17(1593)/§38(3184)/§46(1539)/§47(2354)/
§49(1822)/§50(1088)/§51(1339)/§52(3407) —— 压成"**只留可执行判断 + 指针**", **压缩前原文逐字**搬进
`AGENTS-DETAIL.md` 新 **§D41~§D48**。这 8 条合计 16,606 → 11,820 B。

**为什么是这 8 条**: 头一轮挑了 10 条(含 §42/§43/§44), 但 §17/§42/§43/§44 本来就是我在第八十/八十一轮压过的
**规则型**文本, 再压只省 2.3KB(三条甚至变长); 真正的肥肉是宠物那几条**叙事型**规则 —— §49~§52 一条就 1~3.4KB,
里面混着大量"实测 59.5fps / 40fps / 2085fps / 162 秒"这类**证据**。证据属于 `AGENTS-DETAIL`(§D36~§D40 早有全文),
`AGENTS` 只需留"别用 DirectComposition / 必须带 `WS_EX_NOACTIVATE` / 面板布局只有 `layout()` 一处"这类判断。

**标识符保全**: 落盘脚本对每条做比对(旧条目的每个反引号片段与 ASCII 标识符须在新条目出现), 并**打印"被下沉的标识符"清单**
供人工过目 —— 本次刻意下沉的是 API 细节(`CreateWindowExW` 的完整 flag 串、`GWL_EXSTYLE`、`GetPixel`/`WindowFromPoint`
判据、`dcomp`/`target`/`visual` 等 COM 对象名、语音后端参数), 全部随原文进了 §D41~§D48。三处**关键标识符补回**
(`UIAutomationCore`、`_ipc_reposition`、`update.version_in_text`), 并把 §38 里 `stream` 的 payload 选择规则留在正文。

**数字**: `AGENTS.md` 60,256 → **55,469 B**(对 65,232 的余量 **4,976 → 9,763 B**, 翻倍);
`AGENTS-DETAIL.md` 185,470 → 246,594 B(不参与注入, 无压力)。校验: §5 的 **1..52 条全在**、文件尾完整、
行尾 LF(CRLF=0)、§D41~§D48 八节齐全、§5 标题后无多余空行。

**顺手纠了两个记错的项数**(§4 测试清单): `standalone-plugin-test.py` 6 → **7 项**(第八十七轮加了 `wgpet.py`)、
`pet-overlay-test.py` 72 → **74 项**。

**本轮未动任何代码**, 故未重建 dist。**另记一条待办(留给用户决定)**: `pet-overlay-test.py` 在**干净检出上是红的** ——
A/B 已证死: `plugins\wgpet.dll` **缺席**时 74/74 全绿, **在场**(= master 现状)时 L 段等不到 `WGIME_PET_DUMP_LIVE`
(live dump 只在 Python 版浮层里实现)而报超时。修法很小: 给 `_load_pet_dll()` 加测试钩子(如 `WGIME_PET_NO_DLL=1` →
返回 None), 让该测试在 L 段设它 —— DLL 路径由 `pet-dll-plugin-test.py`(22 项)覆盖, Python 路径由它覆盖。

## 2026-09-24 (第八十七轮补十: 卡通形象对齐参考图 —— 比例 / 遮挡 / 剪影)

用户原话"把卡通的绘制能弄得更像些吗？现在得什么都不像"。查下来**不是细节问题, 是两条结构性的**:

1. **比例错**: 头 1.58×、身体 0.90× ⇒ "大头娃娃 + 一根杆"。改成头 **1.22×**、身体 **1.06×**。
2. **遮挡**: 挎包永远背着, 把身体整个盖住 ⇒ 看起来"一个头 + 一个包"。改成**只有场景 2 背**
   (`Pose.bag`), 其余场景与姿势表都不背 —— 参考图里的柴犬本来就不背包。

顺着参考图的剪影再改: 腿**短而粗**(9.6/10.4 → 12.6/13.4, 描边 +4.0 → +2.4)、
爪子更圆、尾巴**加粗到内孔闭合**(实心蓬松一团, 原来是一圈扁 C 带大洞)、鼻子放大、
描边整体收细(头 2.7→1.9、身体 3.4→2.3、耳/尾 2.4→1.7)。暗面从"一块硬平涂"改成
**两层半透明叠加**(往参考图那种水彩柔过渡靠一点)。

同时按用户给的《柴犬姿势表》做了一整套姿势预设(`wgime_pet_act`: 站/坐/趴/跑/跳/前趴玩/歪头/
闻地面/回头看/睡/打哈欠/眯眼笑)与**姿势表导出**(`%TEMP%\wg-sheet.py` → 网格白底 PNG 2 倍)。

**还差的(照实说)**: ①耳朵仍偏小、贴颅顶, 表里是立起来的明显三角耳; ②水彩柔过渡只做了一半
(仍是平涂 + 一层柔光, 不是真渐变); ③只有侧面视角, 表里的正脸/背面/斜 45° 需要另做几何。

回归: 浮层 21/21、投篮 7/7、面板 16/16、插件路径 7/7。

---

## 2026-09-24 (第八十七轮补九: 场景 1 投篮 + Q版形象 + 吉祥物导出 + 两个真 bug)

- **场景 1(投篮)**: 场地窗 760x360 固定在屏幕中间(角色不漫游 —— 场地跟着窗口走, 一走球架就飘);
  左右各一个球架(立柱+篮板+框+网), 球的状态机: 拍球 → 抛物线飞出 → 进框(框口炸一圈) → 捡回来。
  `arm` 抬爪交给 2 骨 IK 自己折腿, 不用另画投篮姿势。右键可 1→2→3 换场景; 场景 1 里回车=投篮。
- **Q版形象**: 头用 `at_scaled(1.5)`、身体 0.86(大头小身), 头顶呆毛、坐姿(后躯下沉 + 后仰 0.42)、
  赛璐璐平涂暗面、近黑粗描边、项圈加**银环吊坠**。用户随后自己又推了一版 V4(更圆的包子脸 +
  正脸/三分之四都能读), 已一并纳入并跑通全部测试。
- **吉祥物导出**: `wgime_pet_mascot(1)` 让窗口自己填不透明白底。**别想事后抓屏合成** ——
  分层窗的 alpha 抓屏抓不到(`BitBlt` 拿到的是合成后的屏幕, 抓出来是桌面背景)。
- **两个真 bug**: ①**DIB 必须 >= 任何场景的窗口**: 原来是 620x430 而场景 1 的窗是 760 宽,
  ULW 从 DIB 取源取不到右边, 表现是"右边球架整块没画、画面还错位"; 现在 760x440 + 一次性超限告警。
  ②**标定点自愈**(`refine_probe`): 渲染完回读自己那一帧, 就近螺旋找一块纯背毛。形象一改
  (身体变小/坐姿旋转/加暗面)手挑的点就会落到描边或暗面上 ⇒ 逐通道断言假红; 这个坑已踩三次,
  现在让它自己找(实测自动从 (110,186) 挪到 (113,189))。
- 回归: 浮层 21/21、面板 16/16、投篮 7/7、插件路径 7/7、`pet-dll-plugin-test.py` 22/22、undefined-globals 0。

---

## 2026-09-23 (第八十七轮补八: 桌面宠物接进插件体系 —— wgpet.py 优先用 wgpet.dll)

到这一轮为止, Rust 浮层只能在探针/脚本里跑。现在它是**真插件**了:

- `wgime-py-pure\plugins\wgpet.dll`(1.71MB) 跟插件放一起并入库跟踪; `build-package.ps1` 多拷一句 `plugins\*.dll`
  (否则安装布局里没有它, 画面会退回旧的 Python 实现)。`plugins\*.dll` 不参与插件扫描(只认 `*.py`)。
- **分派**: `_window_main()` 先试 `_load_pet_dll()`(文件在不在 → 能不能 `WinDLL` → `abi >= 1`, 任一步不对都写日志返回 None),
  拿到就走 DLL 模式, 拿不到/起不来就**退回原来那套 Python 实现** —— 降级路径保留。
- **游戏式热键沿用同一套契约**: 宿主再拉一次插件 = 新进程 → `_find_overlay()` 找到已有窗口(DLL 版与 Python 版标题相同)
  → `PostMessage(WM_APP_TOGGLE)` → 自己退出。DLL 侧加了 `WM_APP_TOGGLE(0x8002)` 开合面板、
  `WM_APP_ENTER(0x8003)` 甩出选中工具、`WM_APP_ACTIVITY(0x8004)` 打字了精神一下。
- **看门这条不能省**: 插件管理器「结束窗口」是发 WM_CLOSE, 窗口没了但那个 Python 子进程还睡着就是**孤儿进程**
  (§46 的收尾只杀"还有窗口"的进程)。所以 DLL 模式每 0.35s 查一次 `wgime_pet_hwnd()`, 归零就退出。
- 点工具仍**走宿主 API**: DLL 只把 code 通过回调交回 Python, 由 Python 调 `--api run-plugin <code>`(守 §5 规则 51)。

**新回归** `wgime-py-pure\tests\pet-dll-plugin-test.py` **22/22**: S 段(DLL 就在插件旁边/能载入/ABI/导出符号齐/
插件的 `_load_pet_dll()` 真能拿到库/清单没被改坏/工具编码与排序), L 段真机(宿主 API 拉起 → 窗口 300x280 →
再拉一次 620x376 → 第三次收起 → WM_CLOSE 后窗口消失**且子进程退出**)。用户已有宠物窗口时 L 段 SKIP。

---

## 2026-09-23 (第八十七轮补七: 按用户给的参考图改脸 —— 大面罩 / 张嘴吐舌 / 大眼双高光 / 粉内耳)

用户发来一张柴犬参考图说"抄一下这个形象吧"。把参考图里"萌"的来源逐个搬进侧面视角:

| 参考图的特征 | 落到侧面视角 |
|---|---|
| **大白面罩** | 吻部 + 下颊 + 鼻梁一路连到两眼之间; 赤色只剩头顶和眼周那顶"帽子" |
| **张嘴吐舌** | 加了暗色口腔 + 粉舌头(参考图里最抓眼的表情, 光画一条嘴线不够), 新增 MOUTH/TONGUE 两支笔刷 |
| **大眼双高光** | 一大一小两个白点; 高光仍随眼皮一起缩(眨眼不会变墨镜) |
| **粉内耳** | `EAR_IN` 改成粉色 |
| **深色项圈 + 小铜环** | 照做; 宽度从 6.8 收到 4.4 —— 6.8 在屏幕上就是"黑围巾", 与头的轮廓糊在一起 |

**一处有意没抄**: 参考图是正面坐姿, 而这只宠物在屏幕底部**横向走动**, 正面视角没法表现行走;
只搬脸的设计, 姿态仍保持侧面。回归 `verify-overlay.py` 21/21、面板 16/16。

---

## 2026-09-23 (第八十七轮补六: 身体/头/耳再返工 —— 按"屏幕真实大小看剪影")

用户接连指出"身体不像""耳朵和头也很诡异"。三条都改在**剪影**上, 不是细节:

- **身体也改成轮廓路径**(背线平直 + 深胸 + 腰收紧 tuck-up + 后躯圆), 不再是一个大椭圆"面包";
  里白只留胸口一坨 + 腹线一条, 侧面看赤色占主体。
- **头: 吻部长度有甜点区**。两个极端都试过 —— 吻太长像狐狸, 吻太短(颅骨相对变大)像小熊/猫;
  最后定在吻部占头长三分之一强, 并保留**额段(停)**: 没有额段就是一根雪茄, 有了才像狗。
- **耳朵缩小(0.82)并靠近彼此, 耳根落在颅顶**; 头位比背线只高约 10 单位、脖子收细(原来抬太高 + 粗脖子 = "长在粗柄上的头")。

**教训**: 判断形象不该在 3 倍放大图上判断, 要按它在屏幕上的**真实大小**看外轮廓 ——
170px 高的时候细节全没了, 决定"像不像"的只有剪影比例。这几轮我一直在放大图上看, 反而走偏了。

回归: 浮层 21/21、面板 16/16。

---

## 2026-09-23 (第八十七轮补五: 换成柴犬 —— 按识别特征逐条对, 附两个隐蔽 bug)

用户要"柴犬, 要搞得像一点"。柴犬的识别特征比"狗"具体得多, 于是逐条对上:

| 特征 | 做法 |
|---|---|
| **里白(urajiro)** | 吻部/下颊白 + 胸腹白 + **四肢下段白袜子**(大腿赤色、小腿以下里白) + 尾尖白 |
| **眉上两个白点(眉斑)** | 位置两个坑都踩过: 不能画在白底上(白鼻梁铺到额头会把左边那个吞掉)、也不能比耳根高(耳根压额头会把两个都盖住) |
| **卷尾** | 螺旋脊柱(半径 17→15, 扫 200°→-63°)+ 左右轮廓闭合填充。**圈半径必须明显大于尾巴粗细**: 一开始 r=15.5 配半宽 11.4, 内孔半径只剩 4, 整条尾巴糊成"背上一根横香肠" |
| **短吻楔形脸** | 吻尖 30.5 → 27.5(太长就成狐狸了); 杏形上挑眼; 面罩收小, 赤色占主体 |

**两个隐蔽 bug, 比形象本身更值得记:**

1. **眨眼逻辑写反了。** `blink = if blink_t > 0.86 { (blink_t-0.86)/0.14 }` —— `blink_t` 一大就等于 1(闭着),
   而 `blink_t` 大部分时间都在 0.86 以上 ⇒ **眼睛几乎永远半闭**。上一轮我以为"像戴墨镜"只是高光缩放的问题,
   那只是治标; 这轮才找到根: 应该只在 `blink_t < 0.14` 的最后 0.14 秒闭眼。修完眼睛一下就有神了。
2. **cargo 更新不了 `target/release/wgpet.dll` 时竟然不报错。** demo 进程占着 DLL 时, 新构建只写进
   `target/release/deps/wgpet.dll`, 而 `target/release/` 下那个**还是旧的** —— 于是"明明 rebuild 了, 画面却没变",
   我白看了两轮图(改卷尾、改吻部都没进画面)。规矩: 改完先杀掉占着 DLL 的进程, 再核
   `target/release/wgpet.dll` 的时间戳(不是 deps 那个)。

回归: `verify-overlay.py` 21/21、面板 16/16 保持。

---

## 2026-09-23 (第八十七轮补四: 形象返工 —— 头/耳/尾三处按"读成狗"重画)

用户反馈"耳朵位置有点诡异、尾巴不像狗、狗头不像狗"。三条都成立, 而且根子在**画法**而不是参数:

| | 原来 | 现在 |
|---|---|---|
| 狗头 | 一个圆 + 一个奶油色大椭圆当口鼻 → 像海豹/水獭 | **一条侧面轮廓路径**(颅顶圆 → 前额 → **前伸的吻部** → 上唇 → 下巴 → 后脑闭合), 再叠奶油色吻部与额前白斑 |
| 耳朵 | 两个圆椭圆"耷"在头顶 → 像发髻 | **三角立耳**(圆角三角), 远侧耳画在头之前、近侧耳画在头之后, 近侧耳再叠一层粉色内耳 |
| 尾巴 | 一根渐细的线 + 末端一个球 → 像老鼠尾巴 | **羽状尾**: 沿脊柱算左右轮廓(根部细、中后段最蓬松、尖端收)闭合成路径填充 + 描边, 尾尖点一撮白毛 |

顺带修掉一个只有放大看才发现的 bug: **眨眼时眼睛高光点不跟着眼皮缩**, 于是那帧眼睛被压成一条线、
白点却还是原大小 —— 看起来像戴墨镜。现在高光半径与位置都乘 `(1-blink)`。另加了 `wgime_pet_set_blink()`
看画钩子(截图前钉住睁眼, 否则随机抓到眨眼帧)。

形状缓存: 头与耳的形状是固定的(只有位置/角度在变), 所以各建一次路径、之后只用变换摆放; 尾巴每帧重建一条。
回归: `verify-overlay.py` 21/21、面板 16/16 均保持。

---

## 2026-09-23 (第八十七轮补三: 百宝袋做完整 —— 分页 / 悬停动效 / 关闭 / 高度自适应)

用户选了"先把面板做完整"。这一轮把面板从"能点"做成"能日常用":

- **分页**: 每页 12 格(2 列 x 6 行), 超过就分页, 标题条右侧有 `‹ 1/2 ›` 与 `✕`, 也接滚轮
  (但滚轮默认发给**焦点窗**而本窗永不取焦点, 只有 Win10+ 的"悬停时滚动非活动窗口"才送得过来, 所以 `‹ ›` 是主力)。
- **窗口高度按工具数自适应**: 工具少不留一片空白。16 个工具实测 关 280px → 开 423px,
  而**角色脚底在屏幕上的 y 两态都是 2106**(锚点按离窗口*底边*的距离算)。
- **悬停动效**: 每个格子一个 0..1 的值朝目标靠(不是硬切), 用来做"微微放大 + 投影 + 加粗描边";
  实测光标移上去后同一块区域有 5959 个像素变化。
- **关闭**: 标题条 ✕, 以及点面板外任意处收起。

**两条测试纪律(都是真踩出来的)**:
- **凡断言依赖"光标在哪"的, 一律用测试钩子** `wgime_pet_set_cursor()` 钉住光标。这轮悬停断言先红了, 日志里
  命中测试明明打出 `hover -1 -> 0 cursor=(1596,1830) local=(346,113)`, 但**屏幕前的人在动鼠标** ——
  光标自己从 (1596,1830) 漂到 (1507,1868), 等断言去读时早已离开格子。这正是 AGENTS.md 里
  "屏幕前的人一点鼠标就假红"那条, python 版宠物当年也有同样的钩子(`WGIME_PET_FAKE_MOUSE`)。
- **探针要角色的屏幕坐标就用 `wgime_pet_dog_xy()`**, 别拿窗口矩形去猜: 开合面板时锚点从"离底边 240"
  变成"离底边 34", 猜必错一处(这轮也真红了: 关 2112 / 开 2106)。

**回归**: `%TEMP%\wg-rustpet-panel.py` **16/16**(装 16 个工具 / 总页数 2 / 翻页后点第 2 页的格子回调收到 `t12` /
悬停命中与动效 / ✕ 收起 / 点空白收起 / 窗口高度与脚底不动); `verify-overlay.py` 21/21 保持;
`wg-rustpet-e2e.py` 端到端仍通(点工具 → 宿主 `--api run-plugin` → 真多一个工具窗口)。

---

## 2026-09-23 (第八十七轮补二: Rust 宠物做出百宝袋 —— 点工具 → 狗掏出来扔到屏幕中间)

形象与方向经用户确认(奶油色柯基风 + 皮挎包), 接着做场景 2 的核心:**工具面板**。

**已经有了什么**: 宠物在屏幕底部来回走(工作区底边)、会看鼠标、鼠标靠近就跑开、会眨眼摆尾;
游戏式热键呼出**百宝袋**面板(DirectWrite 中文文字 + 2 列格子 + 悬停高亮), 点一格 →
被点中的格子朝角色滑出并缩小("掏出来") → 一个旋转的方块从挎包沿**抛物线**飞到**屏幕正中**
→ **到位那一刻**回调宿主, 由宿主用 `--api run-plugin` 启动工具(守住 §5 规则 51: 插件不许自己跑插件),
落点炸开三圈光晕。

**四个设计决定**(都被实测逼出来):
- **面板与角色共用一个窗**(关 300x280 / 开 620x320)。靠"锚点按离窗口**底边**的距离算"保证开合时
  角色脚底在屏幕上**原地不动**; 用离顶边的距离会让它开面板瞬间下跳 46px。
- **`UpdateLayeredWindow` 的 psize 允许小于 DIB**: DIB 按最大尺寸一次分配, 提交时只交当前那块,
  于是开合面板不必重建 DIB 与渲染目标。
- **飞行物单独一个窗**(220x220, 平时藏着): 它要飞到屏幕正中, 把角色窗撑到半个屏幕的话每帧要提交
  几 MB(120fps = 几百 MB/s); 小窗跟着它走每帧只几十 KB。
- **站着的位置取工作区底边**(`SPI_GETWORKAREA`)而不是屏幕底边 —— 屏幕底边 40px 处已经在任务栏里,
  半透明任务栏会透出底下的东西, 观感像"脚被切了", 而且验收标定点读数会偏一个通道(查了一轮才发现)。

**两个坐标/布局教训**: 面板布局只能有 `layout()` **一处**出处(绘制、命中测试、给探针的矩形共用),
三处各算一遍迟早错位; 点击坐标**统一成屏幕坐标**(`WM_LBUTTONDOWN` 给的是客户端坐标, 要 `ClientToScreen`),
否则真实点击与 `wgime_pet_click` 注入之间会整体偏移。

**验收**(`wgpet-rs/verify-overlay.py` 21/21 保持全绿; 面板链路由 `%TEMP%\wg-rustpet-palette.py` 走通):
面板与 6 个格子矩形正确、悬停 hover=0、点击 → `idx=0` → 飞行物从 (1191,2049) 沿抛物线到 (1920,1080)
→ 回调收到 `jsq`。ABI 升到 2。

---

## 2026-09-23 (第八十七轮: 桌面宠物换 Rust 实现 —— 先把渲染路线用实测走通)

用户判断"python 不适合做这个", 问 rust/C++/Java/Go 选哪个。结论: **Rust**, 而且是零安装成本的选择 ——
`wgpet-rs/` 用 `cdylib` + `windows 0.58` 出 `wgpet.dll`(1.5MB), 而本机 `rustc 1.98 (x86_64-pc-windows-gnu)`
与仓库里早已存在的 `wgime-tsf` 同形态 crate 一起, 说明这条路**根本不用新装东西**。

**这一轮的产出是一个被实测钉死的渲染方案, 不是一个能用的宠物**。踩到并解决的两个大坑:

1. **DirectComposition 是死路**。先按"GPU 合成 + 逐像素 alpha"的正解写了 D3D11 + DXGI 合成交换链 + DComp:
   窗口样式五条齐、120fps、不抢焦点、回读后台缓冲像素 `(250,158,51,255)` 与 `(0,0,0,0)` **分毫不差**、
   `Present` 也一直成功 —— **但屏幕上什么都没有**。两个独立原因叠加:
   - `dcomp`/`target`/`visual` 当时是建窗函数的**局部变量**, 函数一返回就 drop(COM 引用计数归零)⇒ 合成树被拆。
     C++ 样例里它们都是窗口生命周期的成员。**修好这一条画面就出来了**(用户肉眼确认)。
   - 更根本的是: DComp 窗的内容**不按 alpha 做命中测试**, `WindowFromPoint` 照样命中整块窗外框;
     唯一能强制穿透的 `SetWindowRgn` 又会把 DComp 内容**一起裁掉**(装空区域后精灵像素变背景色, 关掉立刻回来)。
     ⇒ 在 DComp 上"看得见"和"点得穿"只能二选一, 与"全屏鼠标穿透"的需求直接冲突。
2. **改用 `UpdateLayeredWindow`(逐像素 alpha 分层窗)** 就同时成立: D2D 的 **DC 渲染目标**画到 32bpp 预乘 DIB,
   再 `UpdateLayeredWindow(ULW_ALPHA)` 提交; 命中测试**逐像素按 alpha** —— 没画到的像素鼠标直接穿过去。
   被动态(`WS_EX_TRANSPARENT`)实测**整窗穿透**(连本体都不挡); 交互态只摘掉这一个样式, 就变成
   **只有画到的像素挡鼠标**、面板外空白照旧穿透、前台全程不变 ⇒ 点工具不丢输入框焦点。

**顺带的三条实测数字**: ULW **不做 vsync 节流**(不设上限跑到 **2085fps**, 纯烧 CPU ⇒ 必须自己限速, 现设 120);
窗口做成**精灵大小并跟着角色走**(260×260), 全屏窗 + 手算脏矩形那一整类残影 bug 因此不存在;
D2D 软件光栅在小窗上完全不构成瓶颈(限速到 120 后稳稳跑满)。

**回归**: `wgpet-rs/verify-overlay.py` **25 项全绿** —— 三层光晕叠在纯品红背景上逐通道整数吻合
(`(255,97,185)` vs 期望 `(255,96,185)`, 证明 `0.35` 叠在 `0.18` 之上)、三层边界各采到中间色(证明真抗锯齿)、
透明处与本体处 `WindowFromPoint` 都跳过本窗、交互态本体可点而空白仍穿透、前后前台不变、120fps、
眼睛采到眼睛色(证明 DIB 没被上下翻转)。探针另加**"环境闸门"**: 屏幕前的人一按截图, 遮罩层把整块读数压成
40% 品红(`(102,0,102)`), 8 条断言集体假红 —— 真踩过, 现在先确认背景色干净再往下断言。

**Rust 侧另两条教训**: `windows 0.58` 的 `CreateSolidColorBrush` 挂在 `Foundation_Numerics` feature 之下
(不打开就是"没有这个方法"); `Present` 返回 `HRESULT` 而非 `Result`(用 `.ok()` 转, 别直接 `?`)。

---

## 2026-09-23 (目标轮次 4: 宿主 API + 宠物形象与动作重做 —— 点工具终于真能跑起来了)

**修掉"点工具没反应"的真因**: 你机器上的日志给出了答案 ——
`tool qrcode -> ok=False ModuleNotFoundError: No module named 'ui'`。发行版是**单文件**,
插件的 `run()` 要 `import ui`/`win`/`wspy`, 而这些只存在于**输入法进程的 sys.modules** 里,
子进程(宠物)永远 import 不到 —— 所以"让宠物自己跑工具"这条路根本不成立。

**新增宿主 API**(`main.py`): `--api list-plugins`(一行 JSON) 与 `--api run-plugin <code>`
(建 Tk root、按 code 加载、跑 run()) —— **和你在输入法里打编码呼出是同一个环境、同一条路径**。
宠物改成把工具交给宿主跑。顺带修掉一个静默 bug: 宠物里 `import plugins` 在发行版下撞到同名的**目录**,
于是两个 `.txt` 插件被无声丢掉(你面板里只有 6 个工具, 就是这个原因)。

**形象与动作重做**(你主要不满意的地方):
* 小狗: 描边+双色明暗+像样比例, 四条腿**四拍走**、尾巴三段摆动、**会眨眼**、会叫(张嘴+声波+小跳)、
  闲了坐下、再闲就睡着冒 zzz; 挎包深棕+亮扣+背带, 打开时露出三个彩色工具。整体放大到 2.05 倍。
* **运动范围/速度**: 从 54px/s 提到 150px/s, 还会**自己偶尔快跑(360px/s)**和小跳, 活动范围 5%~95% 全宽 ——
  以前横穿屏幕要 70 秒, 看着像不动。
* **躲鼠标(以前只有场景 3 有)**: 现在狗也会 —— 240px 内**盯着你看**(头和眼珠跟着光标偏),
  110px 内**掉头小跑躲开**。
* 篮球运动员: 球衣号码 23、五官、双色球鞋, 运球时手跟着球, 回车投篮(弧线+进筐亮起)。
* 四边框场景的主角换成和狗同一套形象, 按所在边框摆姿势(走/爬/倒挂/横跳)。
* **右键点主角就能换场景**(1→2→3→1), 并自动把 `pet_scene` 写进 config.txt —— 以前这个键**没登记进模板**,
  你根本没法发现怎么换场景, 这是我漏的。

**回归**: `pet-overlay-test.py` **72 项全绿**(新增"宿主 API 通"等 3 条, 就是这次事故的守卫);
发行版布局下真机端到端验过: 点第一个工具 -> `交给宿主 API` -> `ok=True 已启动 计算器` -> 窗口真的出现。

---

## 2026-09-22 (目标轮次 3: 场景 1「篮球」+ 场景 3「四边框攀爬 / 跟鼠标躲猫猫」—— 三场景齐了)

**新增场景 1(篮球运动员)**: 主角在屏幕下缘活动, **左右两边各一块篮板/篮筐**(离地 430px)。
你打字他就**原地运球** —— 每 0.9~2.0 秒**随机换手**(那种胯下运球的感觉), 头还朝球那边偏;
闲下来接着溜达。**按回车投篮**: 球划一道弧飞向最近那块篮板, **进筐时篮筐亮起来**并抖出三道声波,
球落地后他再自己捡回来继续运。**呼出工具时按你要的那个手感**: 先把**球砸向屏幕中间**、炸开、才弹出工具面板。

**新增场景 3(四边框 + 躲猫猫)**: 把整圈**周长参数化成一条一维路径**, 于是"绕屏幕四边跑"就只是往前走一个数;
左右边框是**攀爬**(四肢一上一下交替抓), 上方是**倒挂**在"天空"垂下来的藤上, 下方是走路。
**鼠标靠近 300px 就撒腿跑**(往离鼠标远的那头), **贴到 105px 以内就缩起来躲**; 被逼住或者闲得慌了就
**横跳**到离鼠标最远的那段边框 —— 这就把活动范围铺满整屏。鼠标位置只用 `GetCursorPos` 轮询,
**不装钩子、不碰你的鼠标**(测试里也是打桩坐标, 不会真的动你的鼠标)。

**这一轮最值钱的东西其实是一堂课: 读屏幕像素很贵。** 本机实测 `GetPixel` **8.5ms 一次**,
我一开始写的"整条下缘扫一遍"要 **162 秒** —— 结果扫描跑到一半, 被测的宠物**已经到点退出了**,
后面所有断言全变成"假红"(看起来像篮球功能坏了, 其实是我的测试太慢)。中间还误判过两次
(以为宠物卡死、以为球没画出来), 真相是拿**过期的坐标**去扫像素 —— 球运球时纵向峰值 700px/s,
场景 3 横跳时 0.4 秒能差 400px。最后定了四条规矩(已写进 AGENTS.md 规则 50): **先冻住动画再验像素**
(新增 `WM_APP_PAUSE` 测试钩子, 而且必须显式 wp=1/0, 不能用 toggle)、**只扫几十个像素**、
**冻住之后要等位置连续两次不变再采**、**别扫精灵正中心那一行**(球的十字线正好在那儿)。
另外"抓一块到内存再找"那条捷径走不通: `BitBlt(screenDC)` **抓不到这个分层窗**, 只有 `GetPixel` 看得见它。

顺带修掉两处测试脆弱性: 长跑断言从"前台窗口没变过"改成"**本窗不是前台**"(前者会被屏幕前的人
点一下鼠标搞成假红, 真发生过), 以及把 `WM_APP_PAUSE` 从 toggle 改成显式值。

**回归**: `pet-overlay-test.py` **47 → 69 项全绿**(连跑两次), 覆盖三个场景; `standalone-plugin-test.py`
**7/7**(它会把每个插件当独立程序跑一遍); `undefined-globals.py` 0。

---

## 2026-09-22 (目标轮次 2: 桌面宠物 widget —— 场景 2「驮挎包的小狗」跑通, 47 项客观回归 + 变异自检)

用户要一个**全屏、鼠标穿透**的挂件, 最后要成为 wgime-py 的独立插件(可以 dll、不能有 exe)。
第 1 轮把可行性证死并定了内核(见 AGENTS-DETAIL §D36), 这一轮把**插件本体**写出来。

**新增**: `wgime-py-pure\plugins\wgpet.py`(双模式插件, 纯 ctypes/Win32, **完全不用 Tk**)。

- **主角是一只会自己溜达的小狗**, 活动范围 = 屏幕下缘。你在打字时它会站住、抬头、竖起耳朵;
  按回车它会叫一声(三道声波)。
- **挎包里装的正是你自己那套输入法插件**: 面板里列出 8 个工具(jsq/lt/sz/pdf/qrcode/fy + qls/qping),
  是**现读插件目录**得到的 —— 复用宿主的 `plugins.py` 解析器, 不用你手工登记。
- **呼出**: 全局热键 **Ctrl+Alt+P**(走 `RegisterHotKey`, 不装键盘钩子、不抢焦点), 或者**直接点小狗**。
  打开面板时**只去掉 `WS_EX_TRANSPARENT`、保留 NOACTIVATE**: 画出来的面板点得到, 面板外的空白**仍然穿透**,
  而且**前台窗口全程不变** —— 你点工具不会把输入框的光标/焦点弄丢。
- **点一个工具**: 小狗把它从挎包里"叼出来"、划一道抛物线抛到**屏幕中间**、炸开一圈, 然后工具就跑起来了
  (在宠物进程的后台线程里跑: `.py` 插件走 `importlib` + `run()`, `.txt` 插件走宿主 `run_steps`,
  非 low 权限照 §16 先弹确认)。所以**不用改宿主一行代码**, 也不用重建 dist。
- **退不掉的问题顺手解决**: 窗口标题含 `wgpet`, 插件管理器那个「结束窗口」(第八十四轮)能认出它;
  再打一次启动编码 `pet` 则会把"开合面板"转给已经在跑的那个浮层, 不会开第二个。

**怎么做到"浮在所有窗口上面又不打扰你"**(内核实测数据在 §D36): 自己 `CreateWindowExW` 且**创建时**就带
`WS_EX_NOACTIVATE`(Tk 做不到 —— 它的 Toplevel 建窗即 map, 激活已经发生), 键色真透明 + 整屏穿透 + 置顶;
动画用独立线程 + `timeBeginPeriod(1)` + `PostMessage`, 实测 **58.5~58.7fps**。

**回归**: `wgime-py-pure\tests\pet-overlay-test.py` **47 项全过**, 全部是客观判据(不靠肉眼):
`GWL_EXSTYLE` 五条样式齐、`GetForegroundWindow` 前后不变、`WindowFromPoint` 不是本窗、屏幕像素没被糊住、
**在狗身上扫到毛色**(证明真的画上去了)、帧率、狗在走、面板扫得到底色、点格子叼的正是那个工具……
**变异自检 4/4**(把穿透/去重/NOACTIVATE/交互态切换分别改坏, 都被抓到) —— 一个不会失败的测试等于没写。

**下一步**: 场景 1(篮球: 打字运球 / 回车投篮 / 呼出时把球砸向屏幕中间炸开)与场景 3(活动范围扩到屏幕四边框,
左右攀爬、跟鼠标躲猫猫)。

---

## 2026-09-21 (发布 v1.2.15 —— 把"自动更新"发到线上; 发布预检顺手抓出两个会让自动更新失效的真问题)

**发布**: tag `v1.2.15` = 本地 HEAD；三个资产与 `.release-stage-v1215\` SHA256 全同；发布前
`release-assets-check.py --version 1.2.15` **28/28**；`update-test.py` **36/36**(比上一版多 4 条，见下)。

**为什么这版不能省**: 第八十五轮写的自动更新只存在于仓库/新构建的单文件里，而线上最新的 v1.2.14
**还没有这段代码** —— 老用户既不会自动升，也没有"检查更新"可点。所以必须发一版把功能带出去；
同时也验证了"发一版"这件事本身会踩到哪些坑(这版踩到两个，都在下面)。

**① 发布预检抓到的真问题: 单文件"自称旧版本"(老实现的正则被自己的文档抢先命中)**
- 现象(本地预检): 拿 `--version 1.2.15` 跑 `release-assets-check.py`，它报
  `VERSION='1.2.14-py' vs --version 1.2.15` —— 而构建产物里 `main.VERSION` 明明已经是 `1.2.15-py`。
- 根因: `update.version_in_text()` 是"全文搜第一个 `VERSION = '数字...'"。而 `MODULES` 里
  **`update.py` 自己的文档**里就有一句解释转义用的示例 `VERSION = \\'1.2.14-py\\'`，
  它在 `MODULES` 里排在 `main` 之前 ⇒ 抠出来的是**文档里的旧版本号**。
- 后果(很隐蔽，而且正好打在自动更新上): `verify_source()` 会拿它跟 release tag 比 ——
  下载到 v1.2.15 的文件被判"**下载到的是旧版本**"，于是**永远更新不上去**，
  而用户的界面上只会看到"有新版本"却点不动。v1.2.14 时代的"VERSION 陈旧"是同一类问题的另一种形态。
- 修法(两层，缺一不可):
  1. **构建期写一行顶层明文身份**: `build-wgime-pure.py` 从 `main.py` 抠出 `VERSION`，
     在 dist 头部写 `WGIME_VERSION = '<版本>'` —— 它在 repr 之外，**无歧义**；
     `update.version_in_text()` 改成**先认它**，正则只留给"没有这一行的旧文件"兜底；
     发布预检 `release-assets-check.py` 同样先认它。
  2. **文档里不许再出现带数字的 `VERSION = ` 示例**(含本函数自己的 docstring) ——
     兜底正则的"以数字开头"只能挡住 `VERSION = '...'` 那种占位写法，挡不住"举了个真版本号"。
- **回归(这次是真管用的那种)**: `update-test.py` 新增 4 条，其中最关键的一条是
  **拿仓库里真实那个 `dist\wgime-py.py`** 断言"它自称的版本 == `main.py` 里的版本"，
  外加一条"文档里的旧版本号不许抢先命中"。自检: 在**修之前**的 dist 上跑 → 必红
  (`dist='1.2.14-py' main='1.2.15-py'`)，修完 36/36 —— 也就是说这条断言如果早写一天，
  这个 bug 根本到不了发布这一步。(光测假文件是不够的: 假文件里没有"自己文档藏着示例"这个形状。)

**② PDF 预览"取消后第一次必失败"**(详情见下面"第八十五轮补"那条，本版一并带上)

**发版附带动作**:
1. `main.VERSION` → `1.2.15-py`;
2. `wgime-py-pure\build-package.ps1` 重建 dist + package(插件**不进 dist**，所以 pdf.py 的修复
   只跟着 package 走，dist 的变化只有版本号那一行与 `WGIME_VERSION` 新行);
3. `tests\build-release-assets.ps1 -Version 1.2.15` → 三个 zip(第一次打完发现上面的 ①，
...
## 2026-09-21 (第八十五轮补: 修 PDF 预览 —— "刚取消/刚杀过助手"之后第一次预览必失败, 而且能干等几分钟)

**怎么发现的**: 跑全量回归时 `pdf-test.py` 从 78/78 变成稳定 **1/78 红**(`[UI] 预览: 第 1 页渲染进了预览框`)。
先排除"是不是本轮改动引起的": 把 `main.py/tray.py/engine.py/config.txt` 全部 `git stash` 回第八十四轮再跑 —— **一样红**,
说明与自动更新那批改动无关, 是 PDF 插件自己潜伏的时序问题(之前时绿时红, 这次变成了必红)。

**量出来的机制** (探针 `%TEMP%\wg-r85-pdfhang-probe.py`):
* PDF 工具的 WinRT 助手是**常驻子进程**, `ensure()` 里有一条"1 秒防风暴门"(`now - self.started < 1.0` 时不重启);
* 于是**刚 `shutdown()` 过**(PDF 工具的「取消」按钮就是杀助手) 之后的**第一次**调用, 会立刻拿到
  `WinRT 助手还没就绪, 请再试一次` —— 这次预览就废了(实测 0.00s 返回, 不是慢, 是直接失败);
* 而如果它正好落在冷启窗口里, 上限是 `T_COLD=180s`(热态 `T_TIMEOUT=1800s`) —— 预览这种"锦上添花"的调用
  跟着一起干等, 表现就是"预览框里一直写着『渲染预览…』"。
* 测试里的时序恰好命中: 它先 `shutdown()` 两次(测"取消后能自动重启"), 紧接着建窗(几百 ms)后就要预览。

**修法**(`wgime-py-pure\plugins\pdf.py`, 三处, 都在 WinRT 客户端上):
1. `call()` 拆出 `_call_once()`, 并在**"还没就绪"/"超时"**这类可重试失败上**自动重试一次**(0.4s 后再来) ——
   用户点一次「取消」再点预览不再吃假失败; 重试仍失败时把两次原因都报出来;
2. `call(..., timeout=)` 新增**本次上限**参数, 逐级透传给 `render_images(..., timeout=)`;
3. `preview_first_page()` 传 `timeout=20.0` —— 预览卡住就如实说"预览不可用", 不让主界面等 3~30 分钟。

**验证**: `pdf-test.py` **78/78**(连跑 3 次都绿); `package\plugins\pdf.py` 已同步(插件**不进 dist**, 所以
`dist\wgime-py.py` 一字未动 —— 正合 §30 那条"只改插件不用重建 dist")。

**教训**: 常驻助手 + "取消=杀助手"这类设计里, **紧跟取消之后的那一次调用**必须自己会重试; 而且
"锦上添花"的调用(预览/提示)要给**自己的短上限**, 别继承批量任务的长超时。
