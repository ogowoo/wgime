...
**但这条命令**会**永远**命中 `AGENTS.md`/`AGENTS-DETAIL.md`/`CHANGELOG.md` —— 因为**规则正文自己**就写着
`ghp_` 这个字面量（第八十九轮补复验时又白紧张一次）。判"干净"要用真 token 的形状：
`git grep -I -n --cached -E 'ghp_[A-Za-z0-9]{20,}'`（无输出 = 干净）；`ghp_` 那版只当粗筛。
...
- 回归: 柴犬版 `verify-overlay.rs`/`verify-overlay.py` 21/21、面板 16/16。

### 10) 身体与头耳再返工(用户接连指出"身体不像""耳朵和头也很诡异")

- **身体也必须是轮廓路径**(`build_body`, 加进 `Shapes` 缓存): **背线平直 + 深胸 + 腰收紧(tuck-up) + 后躯圆**。
  大椭圆("面包")侧面既没背线也没腰, 读不出品种。里白只留**胸口一坨 + 腹线一条**, 侧面看赤色要占主体。
- **头: 吻部长度有个甜点区**。两轮两个极端都试过 —— 太长(吻尖 30.5)像狐狸, 太短(吻尖 30.5→27.5 又收到 30.5 的
  颅骨比例失衡)像小熊/猫。最后定在**吻部占头长三分之一强**(颅骨 -16..16, 吻 16..34)并保留**额段(停)**。
  没有额段就是一根雪茄; 有了才像狗。
- **耳朵: 缩小(0.82)且靠近彼此**, 耳根落在**颅顶**(不是额头两侧)。原来两耳拉得开、三角比半个头还高,
  用户看到的正是"两片贴上去的纸片"。
- **头位: 比背线只高约 10 单位**, 脖子收细 —— 抬太高 + 粗脖子 = "长在粗柄上的头"。
...
### 12) 场景 1(投篮) + Q版形象 + 吉祥物导出

- **场景 1 是"场地窗"**: 窗口 760x360 固定在屏幕中间, **角色不漫游**(v=0) —— 场地是跟着窗口走的,
  角色一走球架就跟着飘。球架左右各一(立柱 + 篮板 + 框 + 网), 篮框高度 `S1_RIM_Y=96`(离地约 230 单位)。
  球的状态机: 拍球(在爪下弹) → 飞出(抛物线, 顶点约抬 118) → 进球(框口炸一圈) → 捡回来 → 拍球。
  实测球最终落在 (93,96) = 左框中心 ✓。
- **`arm` 抬爪靠 IK 折腿**: `feet()` 多一个 `arm` 参数把近侧前脚抬高, 2 骨 IK 自己会把腿折起来,
  不用另画一套"投篮姿势"。
- **Q版 = 一处缩放**: 头用 `at_scaled(..., 1.5)`, 身体 `place(..., 0.86, ...)`; 再加头顶呆毛、
  坐姿(`sit`: 后躯下沉 + 身体后仰 0.42 弧度)、赛璐璐平涂暗面、近黑粗描边、项圈那枚**银环吊坠**。
- **吉祥物导出**: `wgime_pet_mascot(1)` 让窗口自己填**不透明白底** —— 别想着事后抓屏再合成:
  分层窗的 alpha 抓屏抓不到(`BitBlt` 拿到的是合成后的屏幕), 试过, 抓出来是桌面背景。
- **真 bug: DIB 必须 >= 任何场景的窗口**. `MAX_W/MAX_H` 原本是 620x430, 而场景 1 的窗是 760 宽 ⇒
  ULW 从 DIB 取源取不到右边那一截, 表现是"右边球架整块没画、画面还错位"。现在 760x440,
  并在渲染循环里加了一句一次性告警(超了就记日志), 免得下次再静默出错。
- **标定点自愈**(`refine_probe`): 渲染完**回读自己那一帧的 DIB**, 在标定点附近螺旋搜索一块
  四邻也是纯背毛的点。为什么必须这样: 形象一改(身体变小/坐姿旋转/加暗面), 手挑的点就可能落到
  描边、暗面或挎包上 ⇒ 逐通道断言变**假红** —— 这个坑已经踩过三次(挎包工具的描边、身体旋转、
  以及用户 V4 瘦身), 与其每次改画手调坐标, 不如让它自己找(实测自动从 (110,186) 挪到 (113,189))。
- 回归: 浮层 21/21、面板 16/16、**投篮 7/7**(新的 `%TEMP%\wg-court.py`, 断言场地窗尺寸/居中、
  拍球在弹、投篮进过"飞出"态、抛物线有高点、球最后回到框下)、插件路径 7/7、`pet-dll-plugin-test.py` 22/22。
...
## §D41 §17 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §17 现在只留可执行判断 + 指针）。
...
## §D42 §38 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §38 现在只留可执行判断 + 指针）。
...
## §D43 §46 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §46 现在只留可执行判断 + 指针）。

46. 插件管理器「结束窗口」(第八十四轮)|插件窗口可能活在**独立进程**里(`wgtranslate` 的 detached 子进程), 宿主既没句柄也不知 pid —— 以前只能去任务管理器. 找: `tools.plugin_windows()` = `win.enum_top_windows()`(EnumWindows, 只要可见+有标题) 过滤 `tools.plugin_window_match(title, name, code, filename)`: 文件名主干(`wgtranslate` ← `wgtranslate.py`)/`NAME`/`CODE` 三线索, **短 CODE(<3 字符, 如 `fy`)只认完全相等**(否则标题里含 fy 的窗口会被误认) 且**只收 `python*` 进程的窗口**(`win.process_exe_name`, 标题撞车也不动别人的程序). 结束: `tools.end_plugin_windows()` 先 `win.post_close()`(WM_CLOSE=点 ✕), 等 1.2s 不退再 `win.terminate_process(pid)` —— 后者自带「拒绝 pid<=0 / 拒绝本进程」, 因为 pdf/clock 那种 `run()` 直接建窗的插件窗口就在宿主进程里, 杀进程等于自杀. UI 在插件管理器**底部**那行(顶部按钮条已只剩 10px, 塞不下第 8 个按钮): 「结束窗口」+ 一行灰字提示; 重活在后台线程, 结果 `win.after(0,…)` 回主线程弹气泡(§25). 回归 `tests\plugin-window-test.py`(23 项; 无桌面只跑 S; 开跑前检测到用户的同插件窗口则 A/B/C SKIP, 不动用户的窗口). 边界自检: matcher 永远为真 → 5 红, 去掉 python 安全阀 → 1 红. 布局探针 `%TEMP%\wg-r83-pluginmgr-audit.py`(16 控件不越界/同父兄弟不重叠; 新按钮 x=10 y=410 w=100, 提示 x=118 w=344 右缘 462 < 关闭的 470). 原文 §D32.

## §D44 §47 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §47 现在只留可执行判断 + 指针）。

47. 自动更新(GitHub Releases, 第八十五轮)|只换**纯 Python 单文件** `wgime-py.py`(config/dicts/plugins 一概不碰). 检查 `update.plan()`: `/releases/latest`(404 退 `/releases` 取第一个非 draft/prerelease), 比 `main.VERSION` **严格新**才算; 启动 +7s 后台一次(6h 内不重复), **只发托盘提示**, 查到后 `TRAY.rebuild()` 让菜单那行从「检查更新」变成「更新到 vX…」. 下载两条路互备: `raw`(仓库 `wgime-py-pure/dist/wgime-py.py` ~1.1MB) 优先, 失败退 `asset`(发行包 ~25MB, 核 release body 里那张表的 sha256); 网络 urllib **直连**(`ProxyHandler({})`, 本机 WinINET 代理是黑洞) 失败退 PowerShell `HttpWebRequest`; `update_mirror` 给下载 URL 加前缀. **四条安全线**: 三个标记(`MODULES =`/`PLUGIN_SRC =`/`THIRD_ZIP_B64`)+`compile()`+`>=200KB`、内层 `VERSION` 须比当前新且不低于 tag、asset 路 sha256 须对、下载先落 `.part` 失败即删, 一切先校验再写 `.new`. 替换: detached 小助手(源码走环境变量, 照 §17)用 `OpenProcess(SYNCHRONIZE)+WaitForSingleObject` **等本进程退出** -> 备份 `<file>.bak-<旧版本>` -> `os.replace`(重试 10s) -> pythonw 重启; 本进程发完气泡 `quit_app()`. 只对单文件生效(`update.is_single_file` = 头 8KB 有 `MODULES = `; 源码布局直接拒绝). 配置键 `update_auto/repo/api/mirror/source(默认 raw)/timeout`. **单文件里的 `VERSION` 必须等于发布 tag**(发布预检会拦 —— v1.2.14 时代它还是 1.2.12, 真机验收被守卫拒绝过). 回归 `tests\update-test.py`(36 项, 本地假 GitHub + 真助手 + **真 dist 版本自洽**); 真机探针 `%TEMP%\wg-r85-real-github.py`(真 GitHub, 9 项). **单文件身份认顶层明文 `WGIME_VERSION`**(第八十五轮补起 `build-wgime-pure.py` 从 `main.py` 抠出 `VERSION` 写在 dist 头部, 在 repr 之外): `update.version_in_text` 与发布预检都**先认它**, 正则只兜底旧文件 —— 别再改回"全文搜第一个 `VERSION = '数字...'`": 那会被 `MODULES` 里 update.py **自己文档里的一句版本示例**抢先命中(它在 main 之前), 于是"下载到新版"被判成旧版本, 自动更新**永远装不上**(v1.2.15 发布预检真抓到过). 推论: **任何内嵌 `.py` 的文档里都不许出现带数字的 `VERSION = ` 示例**; 构建期抠不到 `VERSION` 直接拒绝构建. 原文 §D33/§D35.

## §D45 §49 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §49 现在只留可执行判断 + 指针）。

49. 全屏穿透浮层(桌面宠物 widget)必须**创建时就带 `WS_EX_NOACTIVATE`**(目标轮次 1)|自己 `CreateWindowExW(WS_EX_LAYERED|WS_EX_TRANSPARENT|WS_EX_NOACTIVATE|WS_EX_TOOLWINDOW|WS_EX_TOPMOST, WS_POPUP, 0,0,SM_CXSCREEN,SM_CYSCREEN)` + `SetLayeredWindowAttributes(hwnd, 键色, 0, LWA_COLORKEY)` + `ShowWindow(SW_SHOWNOACTIVATE)`. **别用 tkinter 做这个浮层**: Tk 的 Toplevel **建窗即 map**, 激活已经发生 —— "先 withdraw 再 SW_SHOWNOACTIVATE" 实测**照样抢前台**(`stole=1`), 只有"抢了再 `SetForegroundWindow(prev)` 还回去"能救, 但中间 ~0.3s 会把用户正在打的键丢进浮层空窗. 动画用**独立线程 + `timeBeginPeriod(1)` + `PostMessage(WM_APP+n)`** + 主线程 `GetMessage` 阻塞循环(实测 **59.5fps**/wndproc 最慢 1.45ms); **别用 `WM_TIMER`+轮询 sleep**(实测只有 40fps, 是队列被 sleep 饿住, 与渲染无关), 重画只 `InvalidateRect(脏矩形)`. 交互态(要用户点工具时)**只去掉 `WS_EX_TRANSPARENT`、保留 NOACTIVATE**: 画了面板处点得到, 面板外空白**仍穿透**(`LWA_COLORKEY` 连命中测试也透明), 全程**前台不变** ⇒ 点工具不丢输入框焦点. 所有 Win32 函数**必须声明 `argtypes`/`restype`**(64 位句柄按默认 `c_int` 传会截断); `WNDPROC` 的 `WINFUNCTYPE` 对象必须保引用. 客观判据(勿靠肉眼, 探针 4/5 全绿): `GetPixel` 精灵处==颜色(证明分层窗可见)+空白点像素不变(证明真透明)、`WindowFromPoint`≠本窗(真穿透)、`GetForegroundWindow` 前后一致(不抢焦点)、`GWL_EXSTYLE` 四条齐. 交付形态=**`plugins\*.py` 双模式插件 + detached 子进程**(§8.7; `run()` 在宿主 Tk 线程同步跑, 长活窗口必须离开宿主进程), **不需要 dll/exe**. 原型 `wgime-py-pure\testing\pet-probe-{0,1,2}*.py`(草稿目录, 永不入库). 原文 §D36/§D37/§D38.

## §D46 §50 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §50 现在只留可执行判断 + 指针）。

50. 读屏幕像素**绝不允许整屏/整条扫描**(目标轮次 3)|本机实测 `GetPixel` **~8.5ms/次**(4K + DWM + 分层窗在动): 探针里扫一条 `3840x178` 用了 **162 秒**, 直接把被测进程的存活窗口耗光 → 后面所有断言变**假红**(真踩过, 查了半天)。规矩: ①先用测试钩子**冻住动画**(`WM_APP_PAUSE` 且 `wParam=1/0` **显式**指定, 别用 toggle —— 多处调用会错位)再验像素; ②只按状态 dump 里的坐标扫**几十个像素**, 别扫全屏; ③dump 是**状态快照**而 `InvalidateRect -> WM_PAINT` 是**异步**的, 按下暂停后要**等到位置连续两次不变**(`wait_still`)再采, 否则拿到的是上一拍的坐标(主角正在横跳时能差 400px); ④扫精灵别扫它**正中心那一行**(球/圆的十字线正好在那儿, 扫到的是深色线而不是填充色)。**BitBlt 那条路走不通**: `BitBlt(screenDC)` **抓不到这个分层窗**(实测整片空白), 只有 `GetPixel` 看得见它。顺带: 长跑断言要写成"**本窗不是前台**"而不是"前台没变过" —— 后者会被屏幕前的人点一下鼠标搞成假红。

## §D47 §51 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §51 现在只留可执行判断 + 指针）。

51. 插件**不许自己跑插件** —— 要跑就走宿主 API `--api`(第八十六轮)|真事故: 桌面宠物在自己的进程里 import 插件的 run(), 而插件的 run() 要 `import ui`/`win`/`wspy`; **发行版是单文件, 这些模块只存在于宿主进程的 `sys.modules` 里** -> 用户点工具的结果是 `ModuleNotFoundError: No module named 'ui'`, 一个都起不来。规则: ①宿主入口在 `main.py`, 两个命令 `--api list-plugins`(JSON) / `--api run-plugin <code>`(建 Tk root 跑插件); ②**必须在 `_relaunch_if_console_python()` 之前**判掉, 否则 python.exe 启动会被重启成 pythonw 并 detach, CLI 的 stdout 直接丢; 而函数体要放在 `APP_DIR` 算完之后(插件目录要从 BASE/APP_DIR 推); ③别把 API 做在单独文件里: 单文件发行版下 `ui`/`win` 不是磁盘文件, 只有 main.py 自己的 exec 环境里才有; ④插件侧一律 `_host_file()` 找宿主(发行版 `<package>\wgime-py.py`, 源码版 `<wgime-py-pure>\main.py`), `run-plugin` 用 Popen + **stdout 落临时文件**(别用管道: 工具活下来后管道塞满会把工具卡死), 1.2s 内退出的说明起不来; ⑤`--api` 输出一律 UTF-8 显式解码(否则中文名乱码)。回归 `tests\pet-overlay-test.py` 里那条 '宿主 API 通(list-plugins 返回插件清单)' 就是它的守卫。原文 §D39.

## §D48 §52 压缩前的原文（第八十八轮从 AGENTS.md 搬来）
> 压缩前逐字保留（AGENTS.md §52 现在只留可执行判断 + 指针）。
...
### 6) 用户否决两文件方案 ⇒ 重做成**单文件**双模包装（第八十九轮补）

用户看完第一版后给了两条：`之前不是做了一个双模的么？跟随那个来做不行？` + `我也要可以独立运行的` ——
不要"薄插件 + `_pyshot_app.py`"两个文件，要**一个** `plugins\PyShot.py`：既是插件，又能 `python PyShot.py` 独立跑。
（规范 §8.7 的双模式原有两种落地：Tk 窗口类插件走 `_standalone.standalone()`；**非 Tk 程序自己打
`STANDALONE-OK`**，如 `wgpet.py`。本程序是 Qt，属后者。）

**新交付（同一个文件，525,828 B / 10,911 行）**：`plugins\PyShot.py` = 480KB 原程序 + 包装。包装由新脚本
`wgime-py-pure\build-wrap-qt-plugin.py` 生成，`wrap()` 是纯函数：读原单文件 → `ast` 收集**全部** `global` 名 →
`textwrap.indent(…, '    ')` 把整个程序体塞进 `_app_main()` → 拼上清单与双模式头尾块 → 对生成物 `ast.parse` 守卫 →
拒绝"输入=输出"。模块级只剩清单 + `app_version()`/`private_site_dirs()`/`pyside_available()`/`child_env()`/
`spawn_argv()`/`spawn()`/`missing_hint()`/`_tip()`/`run()`。

**为什么不能只是 `if __name__ == '__main__': main()`**（三个硬约束，本轮逐条实测）：
① 模块级 `import PySide6.*` + 模块级 `ensure_deps()` 会在**输入法启动时**被装载器 exec（拖慢启动 / 当场联网装包 /
缺依赖时插件静默消失）⇒ 程序体必须整体进函数，模块级只留 `os/subprocess/sys`；
② 原程序那个 `if __name__ == '__main__':` 块缩进进函数后**照样成立**（独立运行时 `__name__` 仍是 `'__main__'`），
所以**入口仍只有一个** —— §45 的"入口互斥"没被破坏；
③ **`global` 序言是承重的**：原程序 11 处 `global _settings_cache/_current/_cache` 指的是它自己的模块级缓存，
缩进进函数后若不声明，`_app_main` 里的初始化就成了**函数局部**，而嵌套函数里 `global` 读写的是**模块全局**
—— 两个变量，缓存静默失联。

**这一条的实证（A/B，不是"我觉得没问题"）**：给 `_app_main()` 尾部塞一句 `return locals()`，以
`__name__ != '__main__'` exec 整份文件 ⇒ 载荷体全跑一遍（PRELUDE + 74 个 import + 全部类/函数定义）但没有任何
入口副作用，于是能直接检查命名空间：
· 真文件：`_settings_cache`/`_current`/`_cache` **在模块全局里、不在 `_app_main` 的局部里**，
  `current_language()` / `set_language('en', False)` / `_load_settings()` / `tr()` 全部正常且共用同一份状态；
· 对照（把那一行 `global …` 删掉）：三个名字跑进 `_app_main` 局部，`current_language()` **当场 NameError**。
两条已固化成 `tests\pyshot-plugin-test.py` 第 5 节（"删掉序言必红"）。

**包装前安全单（也实测了）**：原程序 `__future__` / `nonlocal` / `globals()` / `eval(` / `multiprocessing` /
`pickle` 皆 0；`exec(` 的 14 处词法命中逐个看过，都是 Qt 的 `menu.exec()/dlg.exec()/app.exec()`（不是内建）；
`__name__` 只有那一处入口块，`sys.modules` 无自引用。另一条前提：**方法体看得见外层函数的局部名**
（`def outer(): X=1; class C: def m(self): return X` 实测返回 1；`classmethod`/`staticmethod`/类体内 lambda 同）
—— 这是整套包法成立的关键，所以类方法读模块级常量不会断。

**实测数字**：`importlib` 装载包装后的文件 **15ms**，且 `PySide6` **不在** `sys.modules`（装载期零重依赖零副作用，
对比：平铺版会在装载时 import 整个 Qt）；`python plugins\PyShot.py` 打 `STANDALONE-OK`；
`--check-deps` 报 `PyShot 2.16.1 … [OK] PySide6 6.11.2`；用 `sys.meta_path` 挡住 `PySide6` 时走
`STANDALONE-SKIP-DEPS` 分支且 **rc=0**（别的机器上不会红）、且**没有 pip 痕迹**（钩子下自动
`PYSHOT_SKIP_DEPS=1`/`PYSHOT_NO_ALERT=1`，绝不联网装包）。

**测试重做**：`tests\pyshot-plugin-test.py` 从 40 项扩到 **70 项**。新增/替换的几组：装载纯净（子进程里量 15ms +
`PySide6` 不在 `sys.modules`）、**真宿主装载器**（`ast.get_source_segment` 把 `main.py` 的 `load_py_plugins`
源码抠出来，喂它一个只放了 `PyShot.py` 的临时插件目录 ⇒ 断言 `'pyshot' in codes` 且装载后 `PySide6` 仍不在
`sys.modules`；喂命名空间要补 `_dfn`（装载器的 debug 日志出口），不然成功路径也会 NameError 掉进 except）、
模块级只许 `os/subprocess/sys` 三个 import 且**不许**调用 `_app_main()`、`global` 序言 A/B、挡住 PySide6 的独立运行、
**生成器回归**（现场合成一个"Qt 式单文件"：模块级 import + 模块级自举 + `global` 缓存 + 尾 `__main__`，
证明"原程序当模块 import 就喷副作用 / 包装后零副作用"且独立运行输出**逐字相同**）、
"同目录里只有一份 `PyShot.py`"（第八十九轮那次覆盖事故的守门员）。`undefined-globals.py` 的 `SKIP_FILES`
由 `plugins\_pyshot_app.py` 改成 `plugins\PyShot.py`（假阳性个数不变，仍是那 7 处）。
...
`pet-dll-plugin-test.py` 22/22；`build-package.ps1` 后 `git checkout -- wgime-py-pure\dist\wgime-py.py`（本轮 `deps.py`
没动 ⇒ dist 只脏 `THIRD_ZIP_B64` 时间戳一行），`%TEMP%\wgime-dist-sync-check.py` → `mismatches=0` /
`main.py embedded match: True`；`package\plugins\` 已同步（旧的两份已删）。

**索引大小写坑（留档）**：Windows 上 git 会把 `plugins/PyShot.py` 当成既有的 `plugins/pyshot.py` 的"修改"，
不动索引就**永远提交不出大写的 `PyShot.py`**。做法：`git rm --cached -- wgime-py-pure\plugins\pyshot.py`
...
### §D55 续：按姿态图重做形象（第九十轮补五）
**素材**：用户给的柴犬**姿态表**（保存在 DSH 附件库 `~\.dsh\attachments\v1\objects\a7\a72481b7…`，
1312×1199）：站立/坐姿/趴卧/活动/其他 五组共 30 个姿势，每行六个视角（正面、正面斜45°、侧面左右、背面、背面斜45°）。
我们引擎只有一种视角（侧面偏三分之四，靠 `face` 镜像），所以**轮廓取侧面、脸取正面斜45°**。

**先把颜色量出来**（别凭眼睛猜）：
```python
# 定点取样(3x3 中位) + 全图颜色直方图
头顶橙 #eb953c / 脸颊奶油 #fdf2e0 / 鼻眼 #0f1111 / 舌头 #f1836c / 耳内 #fbd1aa
描边: 横扫头部取"最暗的暖色像素" -> #814315 / #c16b45 一带, 定 #6b3d1a(暖棕细线)
```
对照旧调色板：旧 OUTLINE `#2e1b13`（近黑）= 用户说的"记号笔"；旧 EAR_IN `#ec8674`（偏红）→ 参考是 `#fbd1aa`（奶油粉）。

**结构上的两个关键判断**（都不是"调数字"能解决的）：
1. **头必须是一条完整轮廓**。原来是"颅骨 + 独立伸出的小吻"两件，吻的贝塞尔收成一个尖 ——
   侧面看像鸟嘴。姿态图的头是"大圆颅 + 一小段短吻"的**一个闭合轮廓**，改成一条路径后立刻对味。
2. **白脸罩是柴犬的脸**。参考图里白色覆盖：颊 + **眼周** + 吻，顶上留一道窄橙 V（额头那道毛）。
   第一版把 V 开得太深（凹到 -10），白脸被劈成两半；第二版把 V 收窄收浅、并把罩子的上沿抬到眼睛之上
   （-19.5），**眼睛才落在白里**。这条是"像不像"的分水岭 —— 眼睛坐在橙毛上就像另一只狗。

**别的定案**：眼睛从"大圆眼 + 双高光"改成**小黑豆 + 单点高光**（参考图眼睛约占头宽 0.18，我们原来 0.25 还带两个高光，
太"迪士尼"）；加**眉上两点**（柴犬的"四眼"）；鼻从大圆鼻头改成**小圆角三角**；耳底宽:高 = 0.8
（旧的是 16:25 ≈ 0.64，纸样上一眼看出是"窄高三角"）。

**工具**：`%TEMP%\wg-svg-parts.py` —— 用 **Inkscape**（`C:\Program Files\Inkscape\bin\inkscape.com`）
把 `art/dog.svg` 的六件摊成一张"纸样" PNG（每件一格 + 名字），**先看纸样再编译**。
从整只狗的画面里反推"这条贝塞尔到底画成什么形状"极费劲；纸样一眼就能看出"头是个带尖嘴的团""耳是个窄三角"。

**坑（留档）**：纸样脚本里 `re.search(r'd="([^"]+)"', tag)` **会命中 `id="head"` 里的 `d="`** ——
抠出来的"路径数据"是字符串 `head`，报错 `这条 path 抠不出坐标: 'head'`。Rust 侧 `svgpath.rs::attr()`
有"前一个字符不能是字母/数字/`-`/`_`"的检查所以没中招；探针改成 `(?:\s)d="`。
**凡按属性名扫 XML 属性，都要防"属性名是另一个属性名的后缀"**（`d=` vs `id=`、`x=` vs `max=`…）。

**验证**：`verify-overlay` 21/21（日志 `svg 用了 6/6 个零件 ... 包围盒差 0(逐件一致)` —— 内置兜底同步改过，
