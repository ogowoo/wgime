...
- 中文标点映射（`MapPunct`，`"`/`'` 自动交替配对；第七十八轮补齐为标准输入法全角表，原 `$`→`¥` 改为 `￥`）：
...
- **与缓存的关系**：烘焙不改内存词典（已经是最新合并结果）。勾选"删除源文件"时，`PrebuildCacheAfterBake` 会用 bake 后的输入重算 `InputMd5` 并复用内存字典直接写 `wgime.mb`，**下次启动命中缓存、跳过冷重建**（~10s → ~1.6s）；不勾选删除源文件则下次启动按新 md5 冷重建一次。重复烘焙幂等。
...
- v 模式：`AddVMode` 对 `^v\d{1,16}$` 生成 大写金额（`UpperAmount`：4 位分组 万/亿/兆/京 + 跨组/组内零规则 + 恒输出"元整"）与千分位（`Thousands`）候选，dynSet 标记不学习；数字键路由（`KeyBordHook.DigitAsCode` + `OnDigit`）：`v\d+` 状态下数字（含 0）持续进码（多位金额可输入）；裸 `v` 有候选（混合/五笔：v=发）时数字优先选候选，`Hook_OnSpaced` 中超出候选个数的数字转为进码（v 模式仍可进入）；裸 `v` 无候选（拼音模式）时数字直接进码。空格上屏大写金额。
...
- **注册表**：`Apps: code -> [名称, 命令, 参数]`。内置编码：`itools`/`tools`（工具箱）、`net`/`wlgj`（网络工具）、`clip`/`jlb`（剪贴板历史）、`bj`/`notes`（便签）、`ys`/`color`（颜色拾取）、`plugins`/`cjgl`（插件管理）；计算器已迁出为 `[csharp]` 插件（`plugins\calc.txt`，`jsq` 直达 + `calc` 别名在 LoadPlugins 后解析）。config.txt 可挂外部程序/目录/网址：`app = 码<TAB>名称<TAB>命令[<TAB>参数]`（TAB 优先；不足 3 段时按空格分隔兜底，含空格的命令用引号；`UseShellExecute` 打开）。
...
- **tools.txt**：bat 同目录，UTF-8。`[tab 名]` 开标签页、`[cols N]` 按钮按 N 列平铺（1-6，默认 2，自动换行）、`[按钮名]` 加按钮、其余行为步骤。步骤动词：`msg`/`confirm`（Yes/No 默认"否"防误触；支持 `| title=`/`| buttons=yesno|okcancel|ok`/`| default=1|2` 扩展，ok 为纯提示不中止）/`run`/`start`（只拉起**不等**退出，常驻程序用；`run` 会干等到超时再收掉子进程）/`shell`/`shellx`（可见控制台交互版）/`open`/`kill`/`wait`/`reg-set`/`reg-del`/`file-del`（通配+递归+根目录保护；占用/无权限条目经 `ToolDel` 单个 try/catch 跳过并记日志，不再中断整步）/`mkdir`；`[shell]...[/shell]`（临时 .cmd，ANSI）与 `[powershell]...[/powershell]`（临时 .ps1，**UTF-8 BOM + 强制 UTF-8 输出**——PS 5.1 无 BOM 按 ANSI 读、重定向时默认 ASCII 会把中文变 `??`，两个坑都已在 `RunScriptBlock` 处理）多行块；交互式变体 `[shellx]`/`[psx]`（`RunVisible` 弹可见控制台，尾部追加 pause/Read-Host 保持窗口，等关闭后记退出码——**脚本内不应再写自己的 pause**，否则要按两次键）。块约定：标记各自独占一行，结束标记缺失会吞掉后续全部内容；单行 PowerShell 同样建议用块（无单行 ps 动词；`shell powershell -Command` 的中文/引号转义在 ANSI/cmd 下易错，块为 UTF-8 BOM 临时 .ps1，中文安全）。解析：`ToolToks`（引号分词）+ `ToolPath`（引号剥离+环境变量展开+**相对路径先按 wgime 目录解析** `ResolveRel`，第九十轮补；纯名字仍走 PATH）+ `ToolRegSplit`（HKCU 等 hive 映射）；执行 `ExecToolStep`（null=成功，`"abort"`=用户取消，其余=错误文本）。
...
| `win.py` | ctypes Win32 封装：文本注入、原生剪贴板读写、单实例互斥体、UIA 光标跟随（**独立 Caret Helper 子进程**：内嵌纯 ctypes 源码经**环境变量**交给子进程 —— 既不落盘 .py、也不进进程命令行，JSONL IPC，主进程绝不初始化 COM/UIA） |
