...
14. **字频(candidate 排序)保留 python 版逻辑**：python 版候选排序用 `语料词频 word_freq + 学习词频 fb×5000`（常见词靠前 + 用户常用词上顶），比 C# 版（只按学习词频 `fb`，语料词频仅造句用）更优，是**有意保留**的决策，不要"对齐"成 C# 版。其余字频机制（学习递增 `+1`、上限 90000/30000、保存取前 20000、50 次或 5s flush、LastPick 置顶、简拼按综合词频排序）与 C# 版一致。
