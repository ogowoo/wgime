...
- 纯 Python 插件（`wgime-py-pure\plugins\*.py`）：契约=模块级 `CODE/NAME/DESC/VERSION/AUTHOR/PERM` + `run()`，窗口用 `ui.py` 设计系统（`make_window/flat_button/rounded_entry/console_text`），不建 `tk.Tk()`、不调 mainloop，定时用 `win.after`，后台线程经 queue+`root.after` 派发。已从 C# 1:1 移植：`clock.py`(悬浮时钟 sz)、`wgime-qr.py`(二维码 qrcode，Nayuki 算法已逐位对齐参考实现)、`wgtranslate.py`(剪贴板翻译 fy, perm=network,run)。插件**不内嵌**：`build-package.ps1` 把 `plugins\*.py` 拷进 `package\plugins\`、把仓库根 `plugins\*.txt`(步骤 DSL) 也拷入，生产环境从外部插件目录加载（`load_py_plugins` 扫 APP_DIR/plugins，`.py` 优先于同 CODE 的 `.txt` 步骤插件）。仓库根 `plugins\` 保留 C# 版插件源(txt)，Python 版源在本目录 `plugins\*.py`。
