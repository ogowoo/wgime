...
python wgime-py-pure\tests\pyshot-plugin-test.py    # PyShot 插件: 薄包装/下划线载荷/不被装载/子进程契约 + 真机 --check-deps（40 项，§53）
```
...
53. 重依赖 GUI 程序(Qt 等)接插件一律「薄包装 + 下划线载荷」(第八十九轮)|PySide6/Qt 这类**不能内嵌**(§12)、import 常在**模块级**、还常带模块级依赖自举、且 Qt 与宿主 Tk 不能共用主线程事件循环 ⇒ **绝不能**直接当 `plugins\*.py`(会在输入法启动时 exec、缺依赖时插件静默消失)。做法: ①载荷(原程序**一个字节不改**)叫 `plugins\_xxx_app.py`(`_` 开头 ⇒ 装载器跳过, 同时进 `tests\undefined-globals.py` 的 `SKIP_FILES` —— 第三方生成物的嵌套作用域会被 symtable 误判); ②薄插件只做: 清单 / 查重依赖 / `subprocess.Popen` 独立进程拉起(优先 `pythonw.exe`+`CREATE_NO_WINDOW`), `run()` 必须**立刻返回**(Tk 主线程同步调); ③缺依赖发气泡给人话(走启动编码 `deps` 或 `pip install`), **别在插件里悄悄装几百 MB**; ④把私有依赖目录 `%LOCALAPPDATA%\wgime-py\site\pip` 塞进子进程 `PYTHONPATH`(否则依赖自检装的包子进程看不见); ⑤独立运行也走载荷, 并留一条**不出界面**的自检路径(如 `--check-deps`)供回归用. ⚠ **别把薄插件命名成与载荷同名**(Windows 大小写不敏感: `pyshot.py` 写下去会**覆盖** `PyShot.py` —— 第八十九轮真踩过, 靠 `C:\Explorer\pyshot\PyShot.py` 副本救回; 回归里有"两文件共存且大小悬殊"的断言守着). 参考 `plugins\pyshot.py`+`_pyshot_app.py`, 回归 `tests\pyshot-plugin-test.py`(40 项), 原文 §D49.
