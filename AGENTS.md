...
36. **托盘图标"个别机器看不见"的根因 = 宿主没装 Pillow（第四十一/四十二轮，别再依赖 PIL）**：
    ...
    （`HKCU\...\NotifyIconSettings\<hash>\IsPromoted`，新机器/新 exe 默认隐藏进 `^`）。
